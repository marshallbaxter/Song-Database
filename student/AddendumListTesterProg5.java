...
public class AddendumListTesterProg5 {
   public static void main(String[] args) throws FileNotFoundException {
      ...
      System.out.println("Comparison to build the Addendum List = "+
      ...
      for (int i=0; i<a.length; i++) {
         if (i != 0)
            System.out.print(","); // commas before all but first
      ...
      while (itr.hasNext()) {
         if (!first)
            System.out.print(",");  // commas before all except first 
         else
            first = false;
   ...
   public static void dump(AddendumList<String> addList){
      for (int i1 = 0; i1 < addList.l1array.length; i1++) {
         ...
         if (l2array == null)
            ...
            Object[] items = l2array.items; // can't cast to array of strings
    ...
    * DO NOT MODIFY I WILL BE USING THIS FOR MY TESTING
    */
   public static void stats(AddendumList<String> addList,
      ...
      for (int i1 = 0; i1 < addList.l1numUsed; i1++) {
         ...
         for (int i2 = 0; i2 < l2array.numUsed; i2++) {
            ...
            if (!addList.contains(item))
               System.err.println("Error: Unexpected failed search in stats");
